...
public class TreeRules
{
    ...
    // Axiom = F+F+
    public Rule[] twistingTree = new Rule[]
    {
        new Rule('F', "F+[F+\\[\\F+/^F+/-F+]/[/F+&\\F++\\F+]C+]")
    };

    // Axiom = +FF+R
    public Rule[] smallTree = new Rule[]{
        new Rule('R', "F+F+F+[F+^-/[F+&R&F+[\\F+R\\^F+C]R[/F+/+F+C]]+F+R+F+]") 
    };

    // Axiom = +FF+R
    public Rule[] twistedLeaningTree = new Rule[]
    {
        new Rule('R', "F+[[++BB\\B]^B]"),
